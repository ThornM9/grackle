...
from .physical_constants import gravitational_constant_cgs, sec_per_year


def evolve_freefall(fc, final_density, safety_factor=0.01, include_pressure=True):
    ...
    freefall_time_constant = np.power(
        ((32.0 * gravitational_constant) / (3.0 * np.pi)), 0.5
    )
    ...
    while fc["density"][0] * my_chemistry.density_units < final_density:
        ...
        dt = safety_factor * np.power(
            ((3.0 * np.pi) / (32.0 * gravitational_constant * fc["density"][0])), 0.5
        )
        ...
        print(
            "Evolve Freefall - t: %e yr, rho: %e g/cm^3, T: %e K."
            % (
...
def evolve_constant_density(
    ...
    while True:
        ...
        fc.calculate_temperature()
        print(
            "Evolve constant density - t: %e yr, rho: %e g/cm^3, T: %e K."
            % (
                current_time * my_chemistry.time_units / sec_per_year,
                fc["density"][0] * my_chemistry.density_units,
                fc["temperature"][0],
