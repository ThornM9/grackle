...
def test_equilibrium():
    ...
    # )
    # axes.loglog(
    #     my_T, -1 * cooling_rate_g, color="black", alpha=0.7, linestyle="-", linewidth=1
    # )
    # axes.xaxis.set_label_text("T [K]", fontsize=fontsize)
    # axes.yaxis.set_label_text(
    #     r"$-\Lambda$ / n${_{\rm H}}^{2}$ [erg s$^{-1}$ cm$^{3}$]", fontsize=fontsize
    # )
    # axes.set_xlim(1e4, 1e9)
    # axes.set_ylim(1e-26, 2e-22)
    # tick_labels = axes.xaxis.get_ticklabels() + axes.yaxis.get_ticklabels()
