...
# Reverse rate: k22
#
# Reaction: (HII + DI <--> HI + DII) (7)
# Forward rate: k50
# Reverse rate: k51
#
# Reaction: (H2I + DII <--> HDI + HII) (8)
# Forward rate: k52
# Reverse rate: k53
#
# Reaction: (H2I + DI <--> HDI + HI) (9)
# Forward rate: k54
# Reverse rate: k55
...
class HI_ODE:
    ...
    def get_rates(self, abundances, T):
        HI, HII, HeI, HeII, HeIII, e, H2I, HM, H2II, DI, DII, HDI, E = abundances
...
class DI_ODE:
    ...
    def get_rates(self, abundances, T):
        ...
        positive_fluxes = [
            self.rates.k2(T) * DII * e,
            self.rates.k51(T) * DII * HI,
            2 * self.rates.k55(T) * HDI * HI / 3,
        ]
        destruction_rates = [
            self.rates.k1(T) * e,
            self.rates.k50(T) * HII,
            self.rates.k54(T) * H2I / 2,
            self.rates.k56(T) * HM,
            cloudy["piHI"],
        ]
...
class DII_ODE:
    ...
    def get_rates(self, abundances, T):
        ...
        positive_fluxes = [
            self.rates.k1(T) * DI * e,
            self.rates.k50(T) * HII * DI,
            2 * self.rates.k53(T) * HII * HDI / 3,
        ]
        destruction_rates = [
            self.rates.k2(T) * e,
            self.rates.k51(T) * HI,
            self.rates.k52(T) * H2I / 2,
        ]
...
class HDI_ODE:
    ...
    def get_rates(self, abundances, T):
        ...
        positive_fluxes = [
            3 * self.rates.k52(T) * DII * H2I / 2 / 2,
            3 * self.rates.k54(T) * DI * H2I / 2 / 2,
            3 * 2 * self.rates.k56(T) * DI * HM / 2,
        ]
        destruction_rates = [
            self.rates.k53(T) * HII,
            self.rates.k55(T) * HI,
        ]
